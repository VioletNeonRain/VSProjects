...
int main()
{
	...
	cout << "确定协议版本成功！" << endl;
	if (initSocket("127.0.0.1", 7611) == 0)  //在初始化socket时第一个参数要记得更改为当前IP地址
	{
		cout << "初始化失败！" << endl;
		return -1;
	}
...
bool initSocket(const char* addr, int port)
{
	...
	if (listen(serverSocket, 100) != 0)  //监听客户端数目上限为100个
	{
...
UINT recvMsg(LPVOID lpvoid)
{
	...
	while (1)
	{
		...
		int i = 0;
		cout << "时间：";
		while (decryptedMsg[i] != '\0')  //根据recv()函数接收到的字符串消息特征来进行输出
		{
			if (decryptedMsg[i] != '\n')
			{
				cout << decryptedMsg[i];
...
void sendMsg(char* msg)
{
	...
	send(clientSocket, encryptedMsg, 1024, 0);  //在上述代码对字符串处理后发送字符串消息
	int i = 0;
